...
import { Package, Users, ShoppingCart, Download, Plus, Settings, Trash2, Eye, LogOut, Store, Phone, Lock, Unlock } from 'lucide-react'
...
export default function Admin() {
  ...
  const actualizarEstadoPedido = async (pedidoId, nuevoEstado) => {
    try {
      // Obtener los datos del pedido antes de actualizar
      ...
      // Enviar WhatsApp automáticamente al usuario
      if (pedidoActual) {
        const pedidoActualizado = { ...pedidoActual, estado: nuevoEstado }
        enviarWhatsAppAlUsuario(pedidoActualizado)
      }
  ...
  const enviarWhatsAppAlUsuario = (pedido) => {
    ...
    if (pedido.estado === 'procesando') {
      ...
      mensaje += `🚚 Tu pedido ha sido enviado y está en camino. ¡Prepárate para recibirlo!`
    } else if (pedido.estado === 'entregado') {
      mensaje += `✨ Tu pedido ha sido entregado. ¡Gracias por tu compra! Si tienes algún problema, contáctanos.`
    } else if (pedido.estado === 'cancelado') {
      mensaje += `❌ Tu pedido ha sido cancelado. Si tienes dudas, contáctanos inmediatamente.`
    } else {
            ...
            <div className="bg-white rounded-lg shadow-md p-6 mb-6">
              ...
              <div className="flex gap-4 flex-wrap">
                ...
                <button
                  ...
                  disabled={loading}
                  className="bg-purple-600 hover:bg-purple-700 text-white font-bold py-3 px-6 rounded-lg transition disabled:opacity-50 flex items-center gap-2"
                >
                  <Download size={18} />
                ...
                <button
                  ...
                  className="bg-green-600 hover:bg-green-700 text-white font-bold py-3 px-6 rounded-lg transition disabled:opacity-50 flex items-center gap-2"
                ...
                <button
                  ...
                  disabled={loading}
                  className="bg-red-600 hover:bg-red-700 text-white font-bold py-3 px-6 rounded-lg transition disabled:opacity-50 flex items-center gap-2"
                >
                  <Trash2 size={18} />
                  Eliminar Todos
                </button>
                        ...
                        <tr key={producto.id} className="border-b hover:bg-gray-50">
                          ...
                          <td className="py-3 px-4 text-center">
                            <button
                              ...
                              className="bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded transition flex items-center gap-2 justify-center"
                            >
                              <Trash2 size={16} />
                              Eliminar
                            </button>
              ...
              {usuarios.length === 0 ? (
                ...
                <>
                            ...
                            <td className="py-3 px-4 text-center">
                              <div className="flex gap-2 justify-center flex-wrap">
                                ...
                                <button
                                  ...
                                  className={`text-white px-3 py-2 rounded transition text-sm flex items-center gap-1 ${
                                    usuario.activo
                                ...
                                <button
                                  ...
                                  className="bg-red-500 hover:bg-red-600 text-white px-3 py-2 rounded transition text-sm flex items-center gap-1"
                  ...
                  <div className="md:hidden space-y-4">
                    {usuarios.map((usuario) => (
                      <div key={usuario.id} className={`bg-white rounded-lg shadow-md p-4 border-l-4 ${!usuario.activo ? 'border-red-500 bg-red-50' : 'border-blue-500'}`}>
                        {/* Nombre y Estado */}
                        <div className="flex justify-between items-start mb-3">
                          <div>
                            <h4 className="font-bold text-gray-800 text-lg">{usuario.nombre || 'Sin nombre'}</h4>
                            <p className="text-xs text-gray-500 font-mono">ID: {usuario.id.substring(0, 8)}...</p>
                          </div>
                        ...
                        {/* Celular */}
                        <div className="mb-3">
                          <p className="text-xs text-gray-600 font-semibold">Celular</p>
                          <p className="text-sm text-gray-800">{usuario.celular || 'Sin celular'}</p>
                        </div>

                        {/* Rol */}
                        <div className="mb-4">
                          <p className="text-xs text-gray-600 font-semibold mb-1">Rol</p>
                          <select
                            value={usuario.rol}
                            onChange={(e) => actualizarRolUsuario(usuario.id, e.target.value)}
                            className={`w-full px-3 py-2 rounded text-sm font-semibold cursor-pointer ${
                              usuario.rol === 'admin'
                                ? 'bg-yellow-100 text-yellow-800'
                                : 'bg-blue-100 text-blue-800'
                            }`}
                        ...
                        {/* Fecha de Registro */}
                        <div className="mb-4">
                          <p className="text-xs text-gray-600 font-semibold">Miembro desde</p>
                        ...
                        <div className="flex gap-2 flex-col">
                          <button
                            onClick={() => abrirModalEdicion(usuario)}
                            className="bg-blue-500 hover:bg-blue-600 text-white px-3 py-2 rounded transition text-sm font-semibold"
                            title="Editar usuario"
                          >
                            ✏️ Editar
                          </button>
                          <button
                            onClick={() => desactivarUsuario(usuario.id, usuario.activo)}
                            className={`text-white px-3 py-2 rounded transition text-sm font-semibold flex items-center justify-center gap-1 ${
                              usuario.activo
                                ? 'bg-orange-500 hover:bg-orange-600'
                                : 'bg-green-500 hover:bg-green-600'
                            }`}
                          ...
                          <button
                            onClick={() => eliminarUsuario(usuario.id)}
                            className="bg-red-500 hover:bg-red-600 text-white px-3 py-2 rounded transition text-sm font-semibold flex items-center justify-center gap-1"
                            title="Eliminar usuario"
                          >
                            <Trash2 size={16} />
                            Eliminar
                          ...
                          <td className="py-3 px-4 text-center">
                            <div className="flex gap-2 justify-center">
                              <button
                                ...
                                className="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded transition text-sm flex items-center gap-1"
                                title="Ver detalles"
                              >
                                <Eye size={16} />
                                Ver Detalles
                              ...
                              <button
                                ...
                                className="bg-green-500 hover:bg-green-600 text-white px-4 py-2 rounded transition text-sm flex items-center gap-1"
